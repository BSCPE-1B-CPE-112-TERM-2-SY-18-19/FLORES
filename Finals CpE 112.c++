...
long armando::wasteful_number(long num2) {	
	n=num2;
	x=n;
	digit=0;
	for (;n!=0;)
...
void armando::exam(long num[1000],long size){
		armando rena;
		if(rena.wasteful_number(size)==1){
			cout<<" Wasteful Number"<<endl;
			cout<<"Sorted: ";
			rena.sorting_ascending(num,size);
			cout<<endl;
		}
		else
		{
...
void armando::sorting_ascending(long num[1000],long size){
		long temp1;
		for(i=0;i<size;i++){
